...
async fn main() -> Result<()> {
    ...
    if fs::metadata("./test.txt").is_ok() {
        write("./test.txt", parsed.clone().map(|tle: TLE| tle.to_string()).collect::<Vec<String>>().join("\n\n"))?;
    } else {
        write("./test.txt", parsed.clone().map(|tle: TLE| tle.to_string()).collect::<Vec<String>>().join("\n\n"))?;
    }
    write("./test.json", serde_json::to_string_pretty(&parsed.collect::<Vec<TLE>>()).expect("Failed to serialise TLE's"))?;
